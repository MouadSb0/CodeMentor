        ...
        <div class="flex justify-between items-center w-full px-6 py-3 max-w-screen-2xl mx-auto">
            <div class="flex items-center gap-8">
                <div class="text-xl font-bold font-headline text-slate-900">
                    <a href="{{ url('/') }}">CodeMentor Player</a>
                </div>
                <nav class="hidden md:flex items-center gap-6">
                    <a class="text-slate-500 hover:text-slate-900 transition-colors font-headline" href="{{ route('dashboard') }}">Dashboard</a>
                    <a class="text-cyan-600 font-semibold border-b-2 border-cyan-600 font-headline" href="{{ route('quizzes') }}">Courses</a>
                </nav>
            </div>
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-2 px-4 py-2 bg-surface-container-low rounded-xl transition-all border-2" :class="timeLeft <= 5 ? 'bg-red-50 border-red-200 text-red-600 animate-pulse' : 'border-transparent'">
                    <span class="material-symbols-outlined transition-colors" :class="timeLeft <= 5 ? 'text-red-600' : 'text-cyan-600'">timer</span>
                    <span class="font-mono font-bold transition-colors" :class="timeLeft <= 5 ? 'text-red-600' : 'text-on-surface'" x-text="timeLeft + 's'"></span>
                </div>
                <button @click="if(confirm('Are you sure you want to submit the quiz now?')) finishQuiz()" class="bg-gradient-to-br from-primary to-primary-container text-white px-5 py-2 rounded-xl font-bold font-headline shadow-sm">
                    Submit Quiz
                </button>
            </div>
        </div>
        ...
        <main class="flex-1 overflow-y-auto p-8 lg:p-12">
            <div class="max-w-4xl mx-auto space-y-8">
                ...
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div class="space-y-1">
                        <span x-text="currentQuestion.category || '{{ $quiz->course_name }}'" class="inline-block px-3 py-1 bg-primary/10 text-primary rounded-full text-xs font-bold tracking-widest uppercase"></span>
                        <h1 x-text="'Question ' + (currentIdx + 1).toString().padStart(2, '0')" class="text-4xl md:text-5xl font-bold text-on-surface leading-tight font-headline"></h1>
                    </div>
                    <div class="flex items-center gap-2">
                        <button class="flex items-center gap-2 px-4 py-2 rounded-xl text-on-surface-variant hover:bg-surface-container-high transition-colors font-medium">
                ...
                <!-- Main Question Card -->
                <div class="bg-surface-container-lowest rounded-[2rem] p-8 md:p-12 shadow-xl border border-outline-variant/10">
                    <div class="space-y-12">
                        <div class="prose prose-slate max-w-none">
                            <p x-text="currentQuestion.question || currentQuestion.text" class="text-xl md:text-2xl text-on-surface leading-relaxed font-medium"></p>
                        </div>

                        <!-- Image Context -->
                        <template x-if="currentQuestion.image">
                            <div class="relative group rounded-2xl overflow-hidden aspect-video bg-surface-container-low max-w-lg mx-auto">
                                <img :src="currentQuestion.image" class="w-full h-full object-cover mix-blend-multiply opacity-80" />
                        ...
                        <template x-if="currentQuestion.type === 'mc' || currentQuestion.type === 'tf'">
                            <div class="space-y-6">
                                <div class="grid grid-cols-1 gap-4">
                                    <template x-for="(option, index) in (currentQuestion.options || ['True', 'False'])" :key="index">
                                ...
                                <div x-show="answered && timeLeft === 0 && selectedAnswer === null" class="p-6 rounded-2xl border-2 bg-red-100 border-red-200 text-red-800">
                                    <p class="font-bold text-lg mb-1">Time's Up!</p>
                                    <p class="opacity-80">You didn't select an answer in time. The correct answer was highlighted above.</p>
                                </div>
                            </div>
                        </template>
                        ...
                        <template x-if="currentQuestion.type === 'text'">
                            <div class="space-y-6">
                                <div class="relative">
                                    <input type="text" x-model="textAnswer" :disabled="answered"
                                        :class="{
                                    ...
                                    <div class="absolute right-4 top-1/2 -translate-y-1/2" x-show="answered">
                                        <span class="material-symbols-outlined text-3xl" :class="isTextCorrect() ? 'text-green-500' : 'text-red-500'" x-text="isTextCorrect() ? 'check_circle' : 'cancel'"></span>
                                    </div>
                                </div>
                                ...
                                <div x-show="answered" class="p-6 rounded-2xl border-2"
                                    ...
                                    <p class="font-bold text-lg mb-1" x-text="timeLeft === 0 && !textAnswer ? 'Time\'s Up!' : (isTextCorrect() ? 'Excellent! Correct Answer.' : 'Not quite. Try again next time!')"></p>
                                    <p class="opacity-80" x-show="!isTextCorrect()">The correct answer was: <span class="font-bold" x-text="currentQuestion.answer || currentQuestion.correct"></span></p>
                                </div>
                            </div>
                        </template>
                    </div>
                ...
                <div class="flex items-center justify-between pt-8">
                    ...
                    <button @click="nextQuestion" :disabled="!answered"
                        class="flex items-center gap-2 px-10 py-4 rounded-2xl font-bold transition-all shadow-xl active:scale-95"
                        :class="answered ? 'bg-primary text-white hover:bg-primary-dim' : 'bg-slate-300 text-slate-500 cursor-not-allowed'">
                        <span x-text="currentIdx === questions.length - 1 ? 'View Final Results' : 'Next Question'"></span>
